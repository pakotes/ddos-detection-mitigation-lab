...
class DatasetPreparationPipeline:
    ...
    def __init__(self):
        ...
        self.processors = {
            ...
            'nf_bot_iot': self.script_dir / "process_nf_bot_iot.py",
    ...
    def check_dataset_availability(self):
        ...
        # NF-BoT-IoT
        nf_bot_iot_dir = self.script_dir / "NF-BoT-IoT-v3"
        csv_files = list(nf_bot_iot_dir.glob("*.csv")) if nf_bot_iot_dir.exists() else []
        if nf_bot_iot_dir.exists() and csv_files:
            available_datasets['nf_bot_iot'] = {
                'path': nf_bot_iot_dir,
                'files': len(csv_files),
                'csv_files': len(csv_files),
                'description': 'Conjunto de dados NF-BoT-IoT para detecção de ataques IoT e DDoS'
            }
            # Verificação do ficheiro de features
            features_file = nf_bot_iot_dir / "NetFlow_v3_Features.csv"
            if not features_file.exists():
                available_datasets['nf_bot_iot']['features_csv_missing'] = True
                available_datasets['nf_bot_iot']['features_csv_warning'] = "AVISO: Ficheiro 'NetFlow_v3_Features.csv' em falta no diretório NF-BoT-IoT-v3. Algumas funcionalidades podem não ser extraídas corretamente."
            else:
                available_datasets['nf_bot_iot']['features_csv_missing'] = False
